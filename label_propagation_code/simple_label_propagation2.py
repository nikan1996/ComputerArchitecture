...
def speedup_label_propagation(F, P, labeled_number, each_i=5):
    ...
    x = 1024*1024*8
    # print('a',a.nbytes/x, 'MB','fu',fu.nbytes/x,'MB','c',c.nbytes/x,'MB')
    print('单进程矩阵运算cache需要:',(a.nbytes+fu.nbytes+c.nbytes)/x,'MB')
    start = time.time()
    while propagation is True:
        round += 1
...
def _propa(a_args, fu_args, c_args,start,end,affinity_core):
    ...
    # print('sizes a fu c',a.shape,fu.shape,c.shape)
    n = fu.shape[0]
    propagation = True
    last_fu = fu
    t = time.time()
    ...
    while propagation is True:
        ...
        new_y = np.dot(x,fu)  + part_c
        loss = np.mean(y - new_y)
        if loss == 0:
            break
        y[:] = new_y
        # print('单次速度:',time.time()-f)
    # print(f'分块并行t:{affinity_core}',time.time()-t)

    shm_close([shm_fu,shm_c,shm_a])
    return
def _paraller_propa(a,fu,c,worker_pool,worker=2,):
    ...
    # label_numer = shape[1]
    data_number = shape[0]
    slice_index=  data_number//2
    start = 0
    end = start+slice_index
    end2 = data_number
    # print('start: end: end2',start,end,end2)
    ...
    # print('wait...')
    # fu2 = _propa(a_args[:3], fu_args[:3], c_args[:3], end, end2, 2)
    # fu2 = _propa(a_args[:3], fu_args[:3], c_args[:3], start, end, 1)
    # fu2 = _propa(a_args[:3], fu_args[:3], c_args[:3], 0, data_number, 2)
...
def parallel_label_propagation(F, P, labeled_number, each_i=5):
    ...
    x = 1024*1024*8
    # print('a',a.nbytes/x, 'MB','fu',fu.nbytes/x,'MB','c',c.nbytes/x,'MB')
    print('并行矩阵运算cache需要:',(a.nbytes/2+fu.nbytes+c.nbytes)/x,'MB')
    _,propagation_time= _paraller_propa(a, fu, c,w_pool)
    # fu = _propa(a,fu,c)
    #第一次传播时间由于进程池需要初始化预热，不统计在加速比内。
    # print(f'并行标签传播花费时间:{propagation_time:.3f}s')
    _,propagation_time= _paraller_propa(a, fu, c,w_pool)
    # print(f'并行标签传播花费时间:{propagation_time:.3f}s')
    fu,propagation_time= _paraller_propa(a, fu, c,w_pool)
    print(f'并行标签传播花费时间:{propagation_time:.3f}s')
...
def parallel_experiment():
    import subprocess
    result = subprocess.run('wmic cpu get L3CacheSize')
    # print('L3Cache:' ,int(result.stdout)/1024,'MB')

    r = subprocess.run('wmic cpu get L2CacheSize')
    # print('L2Cache:' ,int(r.stdout)/1024,'MB')
    ...
    F, P = construct_matrix_F_P(labeled_data=train, unlabeled_data=test)
    print(F.shape)
    print(F.nbytes/1024/1024/8)
    print(P.shape)
    print(P.dtype)
    print(P.nbytes/1024/1024/8)
    _,time1=speedup_label_propagation(F, P, labeled_number,each_i=5)
    _,time2 = parallel_label_propagation(F, P, labeled_number, each_i=5)
    speedup = time1/time2
